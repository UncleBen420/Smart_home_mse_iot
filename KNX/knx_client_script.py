...
def send_knx_request(
    ...
    if tunneling_ack_object.status != 0 :
    ...
    # <- Reciving tunneling request from the gateway
    data_rec5,addr = sock.recvfrom(buf_size)
    tunneling_resp_object = knxnet.decode_frame(data_rec5)
    ...
    # -> sending ack
    tunn_ack_object2 = knxnet.create_frame (
    knxnet.ServiceTypeDescriptor.TUNNELLING_ACK,
    conn_channel_id,
    0,tunneling_resp_object.sequence_counter)
    sock.sendto(
    tunn_ack_object2.frame,
    (gateway_ip, gateway_port))
    
    
    #Receiving the tunneling request from the gateway if it's a read instruction
    response = data;
    if apci == 0:
    	data_rec4,addr = sock.recvfrom(buf_size)
    	tunneling_req_object_res = knxnet.decode_frame(data_rec4)
    	response = tunneling_req_object_res.data
    
	# -> sending disconnect request
    disc_req_objt2 = knxnet.create_frame(
    knxnet.ServiceTypeDescriptor.DISCONNECT_REQUEST,
    conn_channel_id,
    control_endpoint
    )
    logger.debug('>>> DISCONNECT_REQUEST:\n{}'.format(disc_req_objt2))
    sock.sendto(
    disc_req_objt2.frame,
    (gateway_ip, gateway_port)
    )
